                ...
                <div class="row q-col-gutter-x-md">
                    ...
                    <div  class="col-9 comp-grid" >
                        ...
                        <div >
                            ...
                            :bordered="false"
                            :columns="app.menus.emprestimoslistheader" 
                            :rows="records"
                            ...
                            <template v-slot:body="props">
                                <q-tr :class="{selected: isCurrentRecord(props.row)}" :props="props">
                                    <q-td  key="id_usuario" :props="props">
                                        <q-btn v-if="props.row.id_usuario" @click="app.openPageDrawer({ page: 'users/view', url: `/users/view/${props.row.id_usuario}` })" padding="xs" flat color="primary" no-caps >
                                            <q-icon name="visibility" size="xs"  class="q-mr-xs"></q-icon>  {{ props.row.users_nome }}
                                        </q-btn>
                                    </q-td>
                                    <q-td  key="id_inventario" :props="props">
                                        <q-btn v-if="props.row.id_inventario" @click="app.openPageDrawer({ page: 'inventario/view', url: `/inventario/view/${props.row.id_inventario}` })" padding="xs" flat color="primary" no-caps >
                                            <q-icon name="visibility" size="xs"  class="q-mr-xs"></q-icon>  {{ props.row.inventario_nome }}
                                        </q-btn>
                                    </q-td>
                                                ...
                                                <q-menu auto-close transition-show="flip-right"  transition-hide="flip-left" self="center middle" anchor="center middle">
                                                    <q-list dense rounded nav>
                                                        <template v-if="auth.canView('emprestimos/view')">
                                                            <q-item link clickable v-ripple @click="app.openPageDrawer({ page: 'emprestimos/view', url: `/emprestimos/view/${props.row.id}` })">
                                                                <q-item-section>
...
<script setup>
	...
	const props = defineProps({
		...
		routeName : {
			type : String,
			default : 'emprestimoslist',
		},
		apiPath : {
			type : String,
			default : 'emprestimos/index',
		},
		autoLoad: {
			type: Boolean,
			default: true,
